# ici un test global du code
import time
import pyotp

from Security.user_data_manager import add_user, get_user, update_user, set_temporary_password
from Security.authentification import verify_user_credentials
from Security.access_control import has_permission
from Security.logs import log_action
from Security.otp import generate_otp_secret, verify_otp
...
# Réinitialisation du mot de passe de testuser
...
def test_global_workflow():
    ...
    for action in actions:
        ...
        print(f"Action '{action}' -> accès : {allowed}")
        log_action(username, "access_check", details=f"{action}: {allowed}")
    ...
    try:
        ok = verify_otp(username, totp_code)
        print(f"Validation OTP : {ok}")
    except Exception as e:
